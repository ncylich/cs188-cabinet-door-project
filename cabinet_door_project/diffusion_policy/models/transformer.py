...
class VisuomotorTransformerNoiseNet(nn.Module):
    def __init__(
        self,
        action_dim: int = 12,
        state_dim: int = 16,
        ...
        self.time_embed = TimestepMLP(128, d_model)
        self.obs_proj = nn.Linear(state_dim + image_feature_dim, d_model)
        self.action_proj = nn.Linear(action_dim, d_model)
        self.pos_embed = nn.Embedding(n_obs_steps + horizon, d_model)
    ...
    def forward(
        ...
        if image_features is not None:
            if image_features.dim() == 2:
                image_features = image_features.reshape(batch, self.n_obs_steps, -1)
            obs_combined = torch.cat([obs_context, image_features], dim=-1)
        else:
            padding = torch.zeros(batch, self.n_obs_steps, self.image_feature_dim, device=obs_context.device)
            obs_combined = torch.cat([obs_context, padding], dim=-1)

        t_emb = self.time_embed(timesteps).unsqueeze(1)

        obs_tokens = self.obs_proj(obs_combined)
        action_tokens = self.action_proj(noisy_actions)

        seq = torch.cat([obs_tokens, action_tokens], dim=1)
        seq_len = seq.shape[1]
        positions = torch.arange(seq_len, device=seq.device)
        seq = seq + self.pos_embed(positions).unsqueeze(0) + t_emb
