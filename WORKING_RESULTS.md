...
## Phase 14: 100-Episode Validation of Best Model (Completed)

### Setup

Retrained F3 (proprio + handle_pos + handle_to_eef, BC_UNet, 22-dim) from scratch under identical hyperparameters to the ablation sweep (BS=128, LR=1e-3, max 100 epochs, patience=30). Evaluated 100 episodes across 100 distinct kitchens (seed=0–99), 500 steps/episode, 8 parallel workers.

Training: best_ep=4, val=0.3266, 21s — exactly matching the sweep run.

### Results

**44/100 success (44.0%) — PASSED (target: 30+)**

| Metric | Value |
|--------|-------|
| Success rate | **44/100 (44.0%)** |
| Avg distance reduction | ~85% |
| Max DR (Ep 40) | 99% (0.50m → 0.01m) |
| Training time | 21s |
| Eval time | ~2 hours (100 eps × 500 steps, 8 workers) |

Wave-by-wave progression (remarkably stable after wave 1):

| Waves | Episodes | Successes | Running % |
|-------|----------|-----------|-----------|
| 1 | 8 | 6 | 75.0% |
| 1–2 | 16 | 8 | 50.0% |
| 1–3 | 24 | 10 | 41.7% |
...
Failures fall into two clear categories:

1. **Approach failure** (~8 episodes): DR < 30% — policy never finds the handle (ep18: 9%, ep20: 13%, ep26: 8%, ep29: 17%, ep36: 23%, ep79: 8%, ep85: 13%, ep91: 10%). These are kitchen layouts where handle site lookup fails or the cabinet is in an unusual position.

2. **Grasp failure** (~48 episodes): DR 70–97% — policy reaches within 1–10cm of handle but can't complete the pull (ep2: DR=94% but X, ep13: DR=97% but X, ep16: DR=95% but X, ep86: DR=95% but X, ep98: DR=95% but X). The gripper gets to the handle but doesn't apply enough force to open the door.

Successful episodes almost universally have final distance < 5cm (ep4: 0.01m, ep6: 0.01m, ep8: 0.01m, ep11: 0.00m, ep40: 0.01m).

### Key Finding

The true success rate is **44%**, substantially higher than the ablation sweep estimate of 37.5% (6/16). The ablation sweep's 16-episode sample had high variance; 100 episodes gives a stable estimate. The 44% rate is consistent across all 13 waves (σ ≈ 4%), confirming genuine generalization across kitchen layouts.
...
|--------|------------|--------|
| **Success rate (validated, 100 ep)** | **44%** (44/100) | Phase 14: F3 BC_UNet, proprio+handle_pos+handle_to_eef, 22-dim |
| **Success rate (ablation sweep, 16 ep)** | **38%** (6/16) | Phase 13 F3: same config, smaller sample |
| **Success rate (friend's code)** | **10%** (2/20) | Noah: 44-dim handle-relative oracle, BC Transformer |
| **Success rate (our bc_handle.py)** | **5%** (1/20) | `bc_handle.py`: BC Transformer, ep-boundary fix, val=0.0794 |
| **Distance reduction** | **88%** avg (99% peak) | Phase 13 F6 / Phase 14 Ep40 |
...
## Appendix: Experiments / Events Not Covered in Main Phases

### A1: Gripper Binarization Bug Discovery (post-Phase 12)

**What:** A code review (two independent agents) of `bc_handle.py` and `diffusion_policy/evaluation.py` found that the gripper binarization threshold in `dataset_action_to_env_action()` was set to 0.5 on the raw (denormalized) action space — but raw gripper values are `{-1=open, +1=close}`. The training data has 82% open (mean ≈ -0.643), so any model prediction near zero denormalizes below 0.5 → gripper **always stays open**.

**Fix:** Changed threshold from `< 0.5` to `< 0.0` (the midpoint of the `{-1, +1}` space). Same fix applied to the `base_mode` dim.

**Why it ended up this way:** The raw action space values (-1/+1) were treated as if they were probabilities (0/1). The threshold of 0.5 made sense for probabilities but was wrong for signed binary values.

**Impact:** All Phase 1–12 success-rate evals were run with this bug. Every 0% success result before Phase 13 is **invalid** — the robot was physically approaching the handle but the gripper never closed. Distance-reduction metrics are still valid.

---

### A2: Post-Fix Re-Evaluation of Phase 12 Checkpoints

After the gripper threshold fix, the three Phase 12 checkpoints were re-evaluated (20 episodes each):

| Checkpoint | Architecture | Post-fix Eval | Notes |
|-----------|-------------|--------------|-------|
| `bc_handle_best.pt` (12a, single-door criterion) | BC Transformer, 44-dim | **5% (1/20)** | First confirmed success — ep4 at step 175 |
| `bc_unet_best.pt` (12b, U-Net diffusion) | 1D U-Net diffusion, 44-dim | **0% (0/20)** | Gets 3–6cm from handle on 5 eps; val loss 0.053, overfits 107 demos |
| `bc_split_grip_best.pt` (12c, split-gripper) | Split arm+gripper, 44-dim | **5% (1/20)** | Same rate as baseline transformer; no improvement from separate gripper head |

**Why results are low:** The bc_handle.py transformer was the first model trained with handle oracle features and episode-boundary fix — but only 107 demos, early stopping at epoch 24, and the gripper still rarely closes in time. The 5% rate (1/20) has wide confidence interval; P(0/20 | p=0.05) = 36% so seed=1 gave 0/20.

---

### A3: Code Review Agent Findings — Remaining Bugs (Post-Phase 12)

Two agents independently reviewed `bc_handle.py` and `diffusion_policy/` after the gripper fix. Key findings beyond Bug A (threshold):

**Bug B — binary_gripper eval path (`bc_handle.py` line ~1096, not fixed):** When training with `--binary_gripper` flag, the eval worker stores raw sigmoid probability ∈ [0,1] directly as the gripper action instead of converting to {-1, +1}. With threshold-0.0 fix, any prob ≥ 0 → gripper always closes. Does NOT affect `bc_handle_best.pt` (trained without `--binary_gripper`). Setup: code inspection. Result: identified bug, not run as experiment.

**Bug C — 4-kitchen diversity limit (known limitation, not fixed):** Each of 4 workers resets to its own kitchen and reuses that kitchen for all 5 of its episodes via `set_state_from_flattened`. So 20-episode evals actually only test 4 distinct kitchens (5 near-identical episodes per kitchen). This inflates variance — a good kitchen gives 5 successes, a bad one gives 0. True diversity requires 20 separate `env.reset()` calls (~10× slower). Intentional tradeoff for speed.

**Bug D — `set_state_from_flattened` doesn't restore controller state (low priority):** The fast episode reset restores MuJoCo physics state but not the impedance controller's internal targets. `controller.reset()` is called manually but may produce stale goals for the first step. No measurable effect on policy performance observed.

**False positives (verified correct):** obs_deque maxlen, BC alignment (obs[t] → act[t]), split-gripper boolean masking, DDPM/DDIM noise math, quaternion math in preprocess_all_states.py, train/val episode-level split (no leakage), EEF Rodrigues rotation.

---

### A4: MimicGen (mg) Dataset Investigation

**Setup:** Attempted to download the MimicGen-augmented dataset (expected ~5000 demos vs 107 human demos) via `04_download_dataset.py` and manual inspection of `box_links_ds.json`.

**Result:** The `mg` dataset key is registered in RoboCasa's dataset registry metadata but has **no entry** in `box_links_ds.json` (the file mapping dataset keys to Box.com download URLs). The human demos key (`pretrain/atomic/OpenCabinet/20250819/lerobot.tar`) exists; the mg key (`pretrain/atomic/OpenCabinet/20250819/mg/...`) does not. MimicGen data is not publicly hosted through this mechanism as of 2026-03-14.

**Why it ended up this way:** MimicGen data generation requires running the MimicGen pipeline on the base human demos, which is compute-intensive. RoboCasa apparently hasn't released the pre-generated OpenCabinet mg data publicly yet. All experiments use the 107 human demos.

---

### A5: 06_train_policy.py MLP Baseline (Pre-Phase 1 Educational Script)

**Setup:** The starter `06_train_policy.py` script trains a simple 2-layer MLP (BC, no diffusion, no action chunking) on the LeRobot dataset parquet files. Used as the initial sanity check that data loading and training loop work. Default config: 50 epochs, BS=64, LR=1e-3.

**Result:** Loss decreases but the policy fails completely at eval — same proprioception-only limitation as Phase 2 (no vision, no oracle). Documented in CLAUDE.md as "educational only."

**Why it ended up this way:** The script is intentionally minimal to illustrate the training pipeline. It has no access to oracle object positions and uses only the 16-dim `observation.state` proprioception, so the policy cannot locate the cabinet. This is the baseline from which all subsequent oracle/visuomotor work departed.
