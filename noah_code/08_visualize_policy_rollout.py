...
def run_onscreen(model, checkpoint, args):
    ...
    for ep in range(args.num_episodes):
        ...
        for step in range(args.max_steps):
            ...
            action = predict_policy_action(model, checkpoint, obs, history, device)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="Maximum steps per episode before aborting the rollout",
    )
    parser.add_argument(
        "--offscreen",
        action="store_true",
    ...
    model, ckpt, policy_type = load_checkpoint_policy(args.checkpoint, device)
