...
**Task:** `OpenCabinet` in RoboCasa — PandaOmron mobile manipulator (7-DOF Franka arm on Omron wheeled base) must open a kitchen cabinet door in MuJoCo simulation. Each eval episode spawns a random kitchen from 2500+ layout variants.

**Success criteria (two versions in use):**
- `bc_handle.py` / `_any_door_open()`: any door joint ≥ 90% of max normalized travel (robocasa definition)
- `diffusion_policy/evaluation.py` / `check_one_door_success()` (recently added): any hinge joint qpos > 0.3 rad (~17°) — more lenient
...
| `/tmp/diffusion_policy_checkpoints/bc_unet_best.pt` | 1D UNet diffusion | 44-dim oracle | 0.0534 | **0% (0/20)** — got 3-6cm from handle on 5 eps but never grasped; val loss too high, overfit on 107 demos |
| `/tmp/diffusion_policy_checkpoints/bc_split_grip_best.pt` | Split arm+gripper | 44-dim oracle | ? | **5% (1/20)** |
...
| 12b | U-Net diffusion, 44-dim oracle | ~~0/20~~ | **0% (0/20)** — gets within 5cm but can't grasp; U-Net overfits badly on 107 demos |
| 12c | Split arm+gripper arch | ~~0/20~~ | **5% (1/20)** — same as baseline transformer |
